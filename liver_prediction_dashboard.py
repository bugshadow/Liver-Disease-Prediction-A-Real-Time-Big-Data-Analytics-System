    ...
    with placeholder.container():
        ...
        if not df_predictions.empty:
            ...
            with col4:
                ...
                st.metric("Total des Cas", total_pred, delta=f"{total_pred-iteration%10:+d}")
                st.markdown('</div>', unsafe_allow_html=True)
            with col5:
                st.markdown('<div class="metric-card">', unsafe_allow_html=True)
                st.metric("Cas Malades", sick_pred, delta=f"{sick_pred-iteration%5:+d}")
                st.markdown('</div>', unsafe_allow_html=True)
            with col6:
                st.markdown('<div class="metric-card">', unsafe_allow_html=True)
                st.metric("Taux Malade", f"{(sick_pred/total_pred)*100:.2f}%" if total_pred > 0 else "0%", delta="+0.3%")
        ...
        if not df_kafka.empty:
            # Sélection des visualisations pour Kafka
